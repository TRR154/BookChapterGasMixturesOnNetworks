...
NOTE: This code considers the case where we set PRESSURE on the inflow nodes. 
"""
import numpy as np
import matplotlib
...
class Network_2v_time:
    ...
    def __init__(self,file_network:Path,file_data:Path,model:str,
        ...
        nrofpieces = np.ceil(self.pipe_length/candidate_dx)
    ...
    def _read_network_architecture(self,file:Path):
        ...
        # print(self.pipe_length)
        # print(self.pipe_diameter)
        # # print(self.pipe_friction)
        # print(self.pipe_roughness)
    ...
    def _precompute_pressure_law(self):
        ...
        if self.p1_jax is None:
            ...
            if self.model == "speed_of_sound":
                ...
                raise Exception(f"This pressure law is not implented: {self.model}")
    ...
    def convert_boundary_conditions(self):
        ...
        for i,m1 in enumerate(m1_list):
            ...
            #if self.node_id[ index_in] == "entry01":
            #    mu = 0.4
            #    m1 = self.m[i]*mu
            #    m2 = self.m[i]*(1-mu)

            self._precompute_pressure_law()
            p_precomp = lambda rho1,rho2: self.p1_jax(rho1,rho2)+self.p2_jax(rho1,rho2)
            rho_1_0_l,rho_2_0_l,v_0_l = get_bc_data_1v(m1,m2,p_left,a,self.model,T=self.temperature,p_precomp=p_precomp)
            rho_1_0_r,rho_2_0_r,v_0_r = get_bc_data_1v(m1,m2,p_right,a,self.model,T=self.temperature,p_precomp=p_precomp)
    ...
    def discrete_system_jax(self,algebraic:bool,f:float):
        ...
        def F(y,y_old,bc_m1,bc_m2,bc_p1,bc_p2):
            ...
            F contains
                ...
                4. boundary data:
                    - p_i(rho_1,rho_2)on the INFLOW nodes 
                    - m_i=rho_i*v_i on the OUTFLOW nodes
    ...
    def solve(self,algebraic:float,tol:float,scenario:str,f:float):
        ...
        self.u_store = np.zeros(((self.N_time+1),initial_vec.shape[0]))
    ...
    def get_initial_and_bc_data(self,scenario:str):
        ...
        # initial_vec = jnp.zeros(4*(N_x_all_list[i])*len(self.pipe_id))
        ...
        if scenario == "gaslib11_stationary_1v":
            ...
            for i,id in enumerate(self.node_id):
                ...
                if self.node_type[i] in ["source"]:
                    ...
                    index_source_pipe = self.pipe_in.index(id)  
                    rho_1_bc = self.rho_1_0[index_source_pipe][0]
                    rho_2_bc = self.rho_2_0[index_source_pipe][0]
                    ...
                    p1_const = p1_goal*np.ones(N_time-N_time_half)
                    p2_const = p2_goal*np.ones(N_time-N_time_half)

                    bc_p1 = bc_p1.at[:,i].set(np.hstack((bc_p1_int,p1_const)))
                    bc_p2 = bc_p2.at[:,i].set(np.hstack((bc_p2_int,p2_const)))
            ...
            for i,id in enumerate(self.pipe_id):
                ...
                v_1_h = 10**-13*v_source*jnp.ones((N_x_all_list[i]))
                v_2_h = 10**-13*v_source*jnp.ones((N_x_all_list[i]))
    ...
    def save_solution_network(self,algebraic:float,scenario:str,f:float):
        ...
        """
        if scenario == "time_dep":
            file_path = Path(f"save_solution/solutions/network_2v_f_{f}_timedep/pressureinflow/2v_{self.network_data}_dx_{self.candidate_dx}_dt_{self.dt}_T_{self.T}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}.pkl")
            file_path.parent.mkdir(parents=True, exist_ok=True)
            with open(file_path, "wb") as file:
                pickle.dump(self.u_store, file)
        else:
            file_path = Path(f"save_solution/solutions/network_2v_f_{f}_timedep/pressureinflow/2v_{self.network_data}_dx_{self.candidate_dx}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}.pkl")
            file_path.parent.mkdir(parents=True, exist_ok=True)
            with open(file_path, "wb") as file:
    ...
    def load_solution_network(self,algebraic:float,scenario:str,f:float):
        ...
        if scenario == "time_dep":
            file_path = Path(f"save_solution/solutions/network_2v_f_{f}_timedep/pressureflow/2v_{self.network_data}_dx_{self.candidate_dx}_dt_{self.dt}_T_{self.T}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}.pkl")
    ...
    def save_solution_1v_network(self,algebraic:float,scenario:str):
        ...
        """
        if scenario == "time_dep":
            file_path = Path(f"save_solution/solutions/network_1v_timedep/pressureinflow/1v_{self.network_data}_dx_{self.candidate_dx}_dt_{self.dt}_T_{self.T}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}.pkl")
            file_path.parent.mkdir(parents=True, exist_ok=True)
            with open(file_path, "wb") as file:
                pickle.dump(self.u_store, file)
        else:
            file_path = Path(f"save_solution/solutions/network_1v_timedep/pressureinflow/1v_{self.network_data}_dx_{self.candidate_dx}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}.pkl")
            file_path.parent.mkdir(parents=True, exist_ok=True)
            with open(file_path, "wb") as file:
                pickle.dump(self.u_sol, file)
    ...
    def plot_all_stationary(self,algebraic:float,f:float,plot_pipe:bool=False,scenario:str=None,show_labels:bool=False,
        ...
        Args:
            ...
            N (int): number of space discretization points
            algebraic (float): value in front of non linear term
            ...
            f (float): parameter of inner friction
        """
        ...
        self.plot_sol_stationary(prop="m_2",prop_name="$m_2$",unit="$\\frac{kg}{m^3s}$",algebraic=algebraic,
                                 arrows=True,f=f,plot_pipe=plot_pipe,scenario=scenario,show_labels=show_labels,label_node_list=label_node_list,offset_labels=offset_labels,save_legend=save_legend)
        # self.plot_sol_stationary(prop="h_1",prop_name="$h_1$",unit="bar",algebraic=algebraic,save_name_fig=save_name_fig,f=f,plot_pipe=plot_pipe,scenario=scenario,offset_labels=offset_labels)
        # self.plot_sol_stationary(prop="h_2",prop_name="$h_2$",unit="bar",algebraic=algebraic,save_name_fig=save_name_fig,f=f,plot_pipe=plot_pipe,scenario=scenario,offset_labels=offset_labels)
        # self.plot_sol_stationary(prop="h",prop_name="$h$",unit="bar",algebraic=algebraic,save_name_fig=save_name_fig,f=f,plot_pipe=plot_pipe,scenario=scenario,offset_labels=offset_labels)
    ...
    def plot_sol_stationary(self,prop:str,prop_name:str,unit:str,algebraic:float,f:str,arrows:bool=False,plot_pipe:bool=False,scenario:str=None,
        ...
        for i,in_id in enumerate(self.pipe_in):
            ...
            if self.network_name != "one_pipe" and plot_pipe == True:
                ...
                file_path = Path(f"graphics/networks/network_2v_timedep/pressureinflow/{self.network_data}/2v_f_{f}_dx_{self.candidate_dx}_dt_{self.dt}_T_{self.T}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}_PROP_{prop}_pipe_{i}.png")
                file_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        if self.network_name == "one_pipe":
            ...
            ax_pipe.set_xlabel("Distance [m]")
            ax_pipe.set_ylabel(f"{prop_name}")
            file_path = Path(f"graphics/networks/network_2v_timedep/pressureinflow/{self.network_data}/_2v_f_{f}_dx_{self.candidate_dx}_dt_{self.dt}_T_{self.T}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}_PROP_{prop}.png")
            file_path.parent.mkdir(parents=True, exist_ok=True)
            fig_pipe.savefig(file_path)
            plt.close()
        else:
                ...
                for i,node_id in enumerate(self.node_id):
                    ...
                    if offset_labels is not None:
                        if offset_labels[i] is not None:
                            offset_x = offset_labels[0]
                            offset_y = offset_labels[1]
            ...
            save_name = f"graphics/networks/network_2v_timedep/pressureinflow/{self.network_data}/2v_f_{f}_dx_{self.candidate_dx}_dt_{self.dt}_T_{self.T}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}_PROP_{prop}"

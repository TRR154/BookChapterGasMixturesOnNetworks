...
class Network_1v_time:
    ...
    def __init__(self,file_network:Path,file_data:Path,model:str,
        ...
        Then the following paramters are available:
        ## node structure
    ...
    def discrete_system_jax(self,algebraic:bool):
        ...
        def F(y,y_old,bc_m,bc_mu,bc_p):
            ...
            F contains
                ...
                4. boundary data:
                    - p(rho_1,rho_2) on the INFLOW nodes 
                    - mu (mixing ratio) on the INFLOW nodes
                    - m = rho*v on the OUTFLOW nodes
            ...
            for i,id in enumerate(self.node_id):
                ...
                # NOTE: This scenario does not allow a source to have an inflow pipe!!!!
                # NOTE: pipe direction must be equal to flow direction
                ...
                if self.node_type[i] == "sink":
                    ...
                    if n_in_pipes>=1 and n_out_pipes>=1:
                        ...
                        in_pipe_index = in_pipes_index_list[0]
                        rho_1_0_h = y[3*np.sum(N_x_all_list[:in_pipe_index])]
                        rho_2_0_h = y[3*np.sum(N_x_all_list[:in_pipe_index])+N_x_all_list[in_pipe_index]]
                        mass_1_cons_inner += bc_m[i]*(rho_1_0_h/(rho_1_0_h+rho_2_0_h))
    ...
    def solve(self,algebraic:float,tol:float,scenario:str):
        ...
        self.N_time = round(self.T/self.dt) + 1
        ...
        for i in range(self.N_time):
            ...
            for j in range(n_iter):
                ...
                if rhs_norm < tol:
                    print(f" Converged Newton Step {j} with residual norm {rhs_norm}")
                    break
    ...
    def get_initial_and_bc_data(self,scenario:str, algebraic:float=1.0, tol:float=1e-7):
        ...
        if scenario == "one_pipe":
            ...
            # print(get_bc_data_1v(m1=m1,m2=m2,p=p,a=a_list[0]))
            ...
            for i,id in enumerate(self.node_id):
                ...
                if self.node_type[i] in ["source"]:
                    ...
                    p_goal = self.p_jax(rho_1_bc,rho_2_bc)
            ...
            #This scenario loads a previously computed stationary solution using
            #the scenario gaslib40 the initial condition. 
            ...
            try:
                ...
                # raise Exception
                network_stationary = Network_1v_time(self.file_network,self.file_data,self.model,candidate_dx=self.candidate_dx,dt=self.dt,T=self.T)
                network_stationary.solve(candidate_dx=self.candidate_dx,dt=60*24*150*15,T=60*60*24*150*15,algebraic=algebraic,tol=tol,scenario="gaslib40_stationary")
                ...
                if self.node_type[i] in ["source"]:
                    ...
                    def bc_mu_values(t):
                        if t<t_fourth:
                            ...
                            if self.network_name == "one_pipe" or self.network_name == "3mixT":
    ...
    def load_solution_network(self,algebraic:float,scenario:str):
        ...
        if scenario == "time_dep":
            with open(f"save_solution/solutions/network_1v_timedep/pressureinflow/1v_{self.network_data}_dx_{self.candidate_dx}_dt_{self.dt}_T_{self.T}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}.pkl", "rb") as file:
                self.u_store = pickle.load(file)
        else:
            with open(f"save_solution/solutions/network_1v_timedep/pressureinflow/1v_{self.network_data}_dx_{self.candidate_dx}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}.pkl", "rb") as file:
    ...
    def plot_all_stationary(self,algebraic:float,plot_pipe=False,scenario:str=None,show_labels:bool=False,
        ...
        self.plot_sol_stationary(prop="mu",prop_name="$\\mu$",unit="none",
                                 algebraic=algebraic,arrows=True,plot_pipe=plot_pipe,scenario=scenario,show_labels=show_labels,label_node_list=label_node_list,offset_labels=offset_labels,save_legend=save_legend)
        # self.plot_sol_stationary(prop="h",prop_name="$h$",unit="bar",N=N,algebraic=algebraic,save_name_fig=save_name_fig,plot_pipe=plot_pipe,offset_labels=offset_labels)
    ...
    def plot_sol_stationary(self,prop:str,prop_name:str,unit:str,algebraic:float,arrows:bool=False
                            ...
                            label_node_list:list[str]=None,offset_labels:float=None,
        ...
        for i,in_id in enumerate(self.pipe_in):
            ...
            if self.network_name == "one_pipe" or plot_pipe == True:
                ...
                if self.network_name == "one_pipe":
                    ...
                    file_path = Path(f"graphics/networks/network_1v_timedep/pressureinflow/{self.network_data}/1v_dx_{self.candidate_dx}_dt_{self.dt}_T_{self.T}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}_PROP_{prop}.pdf")
                    file_path.parent.mkdir(parents=True, exist_ok=True)
                    ...
                    ax_pipe.set_xlabel("x in m")
                    ax_pipe.set_ylabel(f"{prop_name}")
                    file_path = Path(f"graphics/networks/network_1v_timedep/pressureinflow/{self.network_data}/1v_dx_{self.candidate_dx}_dt_{self.dt}_T_{self.T}_SCENARIO_{self.network_data}_pressure_{self.model}_algebraic_{algebraic}_PROP_{prop}_pipe_{i}.pdf")
                    file_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        if self.network_name != "one_pipe":
            ...
            ax.scatter(np.array(self.node_x_cord)[index_in_nodes],
                     np.array(self.node_y_cord)[index_in_nodes],
                     marker="s", color="b", label="$\\Gamma_q$",s=80,zorder=3)
            ...
            #ax.set_title(f"{prop_name} in {unit} for {scenario} BC")
            save_name = f"graphics/networks/network_1v_timedep/pressureinflow/{self.network_data}/1v_dx_{self.candidate_dx}_dt_{self.dt}_T_{self.T}_SCENARIO_{scenario}_pressure_{self.model}_algebraic_{algebraic}_PROP_{prop}_colourmap"
...
################################  
#END OF DEFINITION OF NETWORK 1V TIMEDEP NETWORK CLASS
################################
...
#     if scenario != "time_dep" and T == int(60*60*12):
#         raise Exception("T is too short.")
#     print("Network is "+str(network.network_name))
#     print("stationary or instationary "+str(stationary_or_instationary))
#     # print(network.network_data)
#     network.solve(candidate_dx=candidate_dx, dt=dt,T=T,algebraic=algebraic,tol=tol,scenario=scenario, multiplicative_factor=multiplicative_factor)
#     network.save_solution_network(algebraic=algebraic, scenario=scenario)
#     if scenario == "gaslib40_stationary":
#         network.plot_all_stationary(algebraic=algebraic,scenario=scenario, plot_pipe=False)
